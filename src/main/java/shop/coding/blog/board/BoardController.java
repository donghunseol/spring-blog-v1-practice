...
public class BoardController {
    ...
    public String update(@PathVariable int id, BoardRequest.UpdateDTO requestDTO, @AuthenticationPrincipal MyLoginUser myLoginUser) { // @RequestBody 파싱이 JSON 방식으로 바꾼다!
    ...
    public String delete(@PathVariable int id, HttpServletRequest request, @AuthenticationPrincipal MyLoginUser myLoginUser) { // body 데이터가 없어서 유효성 검사를 할 필요가 없다!
    ...
    public String save(BoardRequest.SaveDTO requestDTO, HttpServletRequest request, @AuthenticationPrincipal MyLoginUser myLoginUser) {
        ...
        boardRepository.save(requestDTO, myLoginUser.getUser().getId()); // 모델에게 위임한다. '?' 3건을!
    ...
    public String detail(@PathVariable int id, HttpServletRequest request, @AuthenticationPrincipal MyLoginUser myLoginUser) {
        ...
        if (myLoginUser != null && boardUserId == myLoginUser.getUser().getId()) {
            pageOwner = true;
        }
